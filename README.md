...
```text
WSJT-X -> 127.0.0.1:2237 -> WsjtxUdpFanout.exe
                              -> GridTracker       127.0.0.1:2239
                              -> JTSync            127.0.0.1:2238
                              -> WRL CAT Control   127.0.0.1:2240
```
...
- [Windows GUI v2.4.0 installer](downloads/WsjtxUdpFanout-Windows-GUI-v2.4.0-Setup.exe) — Recommended. Standard Windows dashboard with destination management, live statistics, selectable themes, and no command-prompt window.
...
UDP Server port:    2237 (WSJT-X default)
Accept UDP requests: checked
```

`Accept UDP requests` is required if a companion app sends commands back to WSJT-X.

The relay now listens on WSJT-X's default UDP port, so a standard WSJT-X installation does not require a port change. Configure companion applications to use their destination ports shown above.
